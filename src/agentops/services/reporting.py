...
def generate_report_from_results(results_path: Path, output_path: Path | None = None, report_format: str = "md") -> ReportResult:
    ...
    if report_format in ("md", "all"):
        md_path = resolved_output_path if resolved_output_path.suffix == ".md" else resolved_output_path.with_suffix(".md")
