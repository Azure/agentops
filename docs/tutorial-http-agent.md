...
```
.agentops/
├── run-http.yaml                       ← HTTP run config
├── bundles/agent_http_baseline.yaml   ← baseline evaluators (IntentResolutionEvaluator, TaskCompletionEvaluator, CoherenceEvaluator, avg_latency_seconds; ToolCallAccuracyEvaluator disabled by default)
├── datasets/smoke-http.yaml           ← smoke dataset config
└── data/smoke-http.jsonl              ← 5 generic Q&A rows
```
...
Open `.agentops/run-http.yaml`. The starter config already points at the HTTP bundle and smoke dataset:

```yaml
version: 1
bundle:
  path: bundles/agent_http_baseline.yaml
dataset:
  path: datasets/smoke-http.yaml
backend:
  type: http
...
| *(extra JSONL fields)* | auto | All row fields except `input_field`/`expected_field` are forwarded automatically — `session_id`, `user_id`, `context`, `tool_definitions`, etc. |
