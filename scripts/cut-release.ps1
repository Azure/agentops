...
#   7. Syncs VSIX version in plugins/agentops/package.json
...
# PowerShell does not abort on a failing native command, so check the exit
# code explicitly and stop before anything is branched, committed, or pushed.
Write-Host "`n>>> Checking CHANGELOG [Unreleased] section..." -ForegroundColor Yellow
python scripts/check_changelog.py check-unreleased
if ($LASTEXITCODE -ne 0) {
    Write-Error "CHANGELOG [Unreleased] check failed. Fix the changelog before cutting a release."
    exit 1
}
...
# ── Step 8: Configure git, commit and push ──────────────────────────
